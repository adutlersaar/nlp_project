...
def evaluate_on_aug(data_dir='data', pretrained_weights='bert-base-uncased'):
    ...
    bart_texts = bart_df[~bart_df['text'].isna()].groupby('index')['text'].apply(list).reset_index().rename(columns={'text': 'bart_text'})
    t5_texts = t5_df[~t5_df['text'].isna()].groupby('index')['text'].apply(list).reset_index().rename(columns={'text': 't5_text'})
    ...
    with torch.no_grad():
        for model_name in saved_models:
            ...
            for text, t5_text, bart_text in tqdm(X_test):
                y_pred.append(model(**tokenize(tokenizer, {'text': text}).to(device))[0].softmax(dim=1).squeeze().cpu().numpy())
                y_t5_pred.append(model(**tokenize(tokenizer, {'text': t5_text[0]}).to(device))[0].softmax(dim=1).squeeze().cpu().numpy())
                y_bart_pred.append(model(**tokenize(tokenizer, {'text': bart_text[0]}).to(device))[0].softmax(dim=1).squeeze().cpu().numpy())
            del model
